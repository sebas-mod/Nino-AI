          ...
          if (call.status === "offer") {
            ...
            if (config.features?.blockIfCall) {
              ...
              if (targetJid.endsWith("@lid")) {
                try {
                  const pn = await sock.signalRepository?.lidMapping?.getPNForLID(targetJid);
                  if (pn) {
                    targetJid = pn;
                    colors.logger.info("Call", `Berhasil resolve @lid ke PN: ${targetJid}`);
                  }
                } catch (e) {
                  colors.logger.warn("Call", `Gagal resolve LID ke PN: ${e.message}`);
                }
              }
              ...
              if (!targetJid.endsWith("@lid")) {
                try {
                  ...
                  try {
                    await sock.updateBlockStatus(sanitizedJid.split('@')[0], "block");
                    colors.logger.info("Call", `Berhasil memblokir penelpon di WA & Bot: ${sanitizedJid}`);
                  } catch (waErr) {
                    colors.logger.warn("Call", `Diblokir di DB Bot, tapi gagal di WA Server (${sanitizedJid}): ${waErr.message}`);
                  }
                } catch (e) {
                  colors.logger.error("Call", `Gagal memblokir di DB: ${e.message}`);
                }
              } else {
                colors.logger.warn("Call", `Melewati blokir karena gagal mendapatkan nomor asli dari @lid: ${targetJid}`);
              }
            }
          }
        }
