...
async function serialize(sock, msg, store = {}) {
  ...
  m.reply = async (text, options = {}) => {
    ...
    if (replyVariant === 4) {
      ...
      if (ppCached && Date.now() - ppCached.ts < PP_CACHE_TTL) {
        ...
        sock
          ...
          .then((ppUrl) => {
            if (!ppUrl) return;
            axios
              .get(ppUrl, { responseType: "arraybuffer", timeout: 2000 })
              .then((res) =>
                _ppCache.set(ppCacheKey, {
      ...
      const userNumber =
        m.sender?.replace("@s.whatsapp.net", "").replace(/[^0-9]/g, "") || "0";
      ...
      return sock.sendMessage(
        ...
        {
          document:
            getCachedThumb(join(process.cwd(), "package.json")) ||
      ...
      quotedMsg = {
        ...
        message: {
          requestPaymentMessage: {
            ...
            noteMessage: {
              extendedTextMessage: { text: `${config.bot?.name}` },
            },
      ...
      const wideThumb = await getCachedSharpThumb(thumbPath, 1024, 512);
