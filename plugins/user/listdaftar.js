...
const pluginConfig = {
  ...
  usage: ".listdaftar [page <numero>] [search <nombre>] [sort terbaru]",
...
async function handler(m, { sock }) {
  ...
  if (registeredUsers.length === 0) {
    return m.reply(`❌ Todavía no hay usuarios registrados!`);
  }
  ...
  text += `> Orden: *${options.sort === "terbaru" ? "Recientes" : "Default"}*\n`;
  if (options.search) {
