...
const pluginConfig = {
    ...
    category: 'sticker',
    description: 'Cambia el packname y el autor del sticker',
...
async function handler(m, { sock, config: botConfig }) {
    ...
    if (!quoted) {
        return m.reply(
            `🖼️ *sᴛɪᴄᴋᴇʀ ᴡᴀᴛᴇʀᴍᴀʀᴋ*\n\n` +
            `> Responde a un sticker con el caption:\n` +
            `> \`${m.prefix}swm packname\`\n\n` +
            `*ᴄᴏɴᴛᴏʜ:*\n` +
            `> \`${m.prefix}swm Ourin-AI\`\n` +
            `> \`${m.prefix}swm Ourin-AI|LuckyArchz\` _(packname + autor)_`
        )
    }
    
    const isSticker = quoted.type === 'stickerMessage' || quoted.isSticker
    if (!isSticker) {
        return m.reply(`❌ *ɢᴀɢᴀʟ*\n\n> Responde a un mensaje de sticker, no ${quoted.type?.replace('Message', '') || 'otro medio'}`)
    }
    ...
    if (!input) {
        return m.reply(
            ...
            `*ᴄᴏɴᴛᴏʜ:*\n` +
            `> \`${m.prefix}swm Ourin-AI\`\n` +
            `> \`${m.prefix}swm Ourin-AI|LuckyArchz\` _(+ autor)_`
        )
    }
