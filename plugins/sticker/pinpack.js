...
const pluginConfig = {
  ...
  category: "sticker",
  description: "Busca imagenes de Pinterest y conviertelas en un pack de stickers",
  usage: ".pinpack <query>",
  example: ".pinpack cat",
  isOwner: false,
  isPremium: false,
...
async function handler(m, { sock }) {
  ...
  if (!query) {
    return m.reply(
      `── .✦ 𝗣𝗜𝗡 𝗣𝗔𝗖𝗞 ✦. ── 𝜗ৎ\n\n` +
        `Busca imagenes de Pinterest → conviertelas en un pack de stickers!\n\n` +
        `╭─〔 Como usar 〕───⬣\n` +
  ...
  try {
    ...
    await m.reply(
      `── .✦ ──\n\n> Descargando *${results.length}* imagenes de Pinterest\n> Luego se convierten en un pack de stickers... .☘︎ ݁˖`,
    );
    ...
    if (!stickerBuffers.length) {
      await m.react("✘");
      return m.reply(`── .✦ ──\n\n> No se pudieron descargar las imagenes .☘︎ ݁˖`);
    }
    ...
    try {
      await sock.sendStickerPack(m.chat, stickerBuffers, m, {
        ...
        publisher: author,
        author,
        description: `Pack de stickers de Pinterest: ${query}`,
        emojis: ["❤"],
      });
      await m.react("✓");
    } catch (packErr) {
      ...
      await m.reply(
        `── .✦ ──\n\n> El pack fallo, enviando uno por uno... .☘︎ ݁˖`,
      );
      ...
      if (sent > 0) {
        await m.react("✓");
        await m.reply(
          `── .✦ ──\n\n> Enviados correctamente *${sent}* stickers de *${packname}* .☘︎ ݁˖`,
        );
      } else {
        await m.react("✘");
