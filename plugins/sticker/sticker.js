...
const pluginConfig = {
    ...
    category: 'sticker',
    description: 'Crea un sticker desde imagen/video con opciones de crop/resize',
...
async function handler(m, { sock, config: botConfig }) {
    ...
    if (!isImage && !isVideo) {
        await m.reply(
            `🖼️ *sᴛɪᴄᴋᴇʀ ᴍᴀᴋᴇʀ*\n\n` +
            `Envia/responde una imagen o video con el caption:\n` +
            `\`${m.prefix}s\`\n\n` +
            `*ᴏᴘsɪ:*\n` +
            `> \`--crop\` - Recortar a cuadrado\n` +
            `> \`--resize WxH\` - Redimensionar al tamano\n` +
    ...
    try {
        ...
        if (isVideo) {
            ...
            try {
                ...
                if (duration > 10) {
                    await m.reply(`❌ Video demasiado largo!\n\n> Duracion: ${duration.toFixed(1)} segundos\n> Maximo: 10 segundos`)
                    await m.react('☢')
