...
const pluginConfig = {
  ...
  category: "sticker",
  description: "Busca y envia packs de stickers",
...
class StickerAPI {
  async search(query, page = 1) {
    try {
      if (!query) throw new Error("Consulta vacia");
...
async function handler(m, { sock }) {
  ...
  if (!query) {
    return m.reply(
      `── .✦ 𝗦𝗧𝗜𝗖𝗞𝗘𝗥 𝗣𝗔𝗖𝗞 ✦. ── 𝜗ৎ\n\n` +
        `Busca y envia packs de stickers!\n\n` +
        `╭─〔 Como usar 〕───⬣\n` +
  ...
  try {
    ...
    if (!search.status || !search.data?.length) {
      await m.react("✘");
      return m.reply(
        `── .✦ ──\n\n> No hay packs de stickers para: *${query}* .☘︎ ݁˖`,
      );
    }
    ...
    if (!detail.status || !detail.stickers?.length) {
      await m.react("✘");
      return m.reply(`── .✦ ──\n\n> No se pudo obtener el detalle del pack de stickers .☘︎ ݁˖`);
    }
    ...
    try {
      await sock.sendStickerPack(m.chat, stickerBuffers, m, {
        ...
        publisher: author,
        author,
        description: `Pack de stickers: ${packname}`,
        emojis: ["❤"],
      });
      await m.react("✓");
    } catch (packErr) {
