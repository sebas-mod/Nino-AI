...
const pluginConfig = {
    ...
    alias: ['attp2', 'attp3'],
    category: 'sticker',
    description: 'Crea un sticker de texto animado',
    usage: '.attp <texto>',
    example: '.attp Hola Mundo',
    isOwner: false,
    isPremium: false,
    isGroup: false,
    isPrivate: false,
...
async function handler(m, { sock }) {
    ...
    if (!text) {
        return m.reply(
            ...
            `> Ejemplo: \`${m.prefix}attp Hola Mundo\``
        )
    }
    if (text.length > 100) {
        return m.reply(`❌ Texto demasiado largo. Maximo 100 caracteres.`)
    }
    m.react('🕕')
    try {
        ...
        if (!stickerRes) throw new Error('No se pudo descargar el sticker del servidor')
