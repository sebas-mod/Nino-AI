...
const loveQuotes = [
  "El amor verdadero no conoce distancia 💕",
  "Dos corazones unidos no se separan 💗",
  "Ustedes son como un rompecabezas perfecto 🧩",
  "Pareja hecha en el cielo! ✨",
  "La quimica es muy fuerte! 🔥",
  "Ustedes son metas de pareja 💑",
  "El destino los unio 🌟",
  "Compatibilidad perfecta detectada! 💘",
];
...
const compatibilityText = (percent) => {
  ...
  if (percent >= 30) return "Vale la pena intentarlo 💓";
  return "Necesita mas esfuerzo 💔";
};
...
async function handler(m, { sock }) {
  ...
  if (memberJids.length < 2) {
    return m.reply("❌ *ɢᴀɢᴀʟ*\n\n> Debe haber al menos 2 miembros para emparejar!");
  }
  ...
  if (registrationRequired && registeredMembers.length < 2) {
    return m.reply(
      "❌ *ɢᴀɢᴀʟ*\n\n> El modo de registro obligatorio esta activo. Debe haber al menos 2 miembros registrados en este grupo!",
    );
  }
  ...
  text += `╰┈┈┈┈┈┈┈┈⬡\n\n`;
  if (usedRegistration) {
    text += `> ✨ _Emparejados segun los datos de registro_\n`;
  }
