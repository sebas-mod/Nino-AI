...
const pluginConfig = {
    ...
    alias: [],
    category: 'fun',
    description: 'Elige al azar un miembro para cierta categoria',
    usage: '.<categoria>',
    example: '.ganteng',
    isOwner: false,
    isPremium: false,
...
async function handler(m, { sock }) {
    ...
    try {
        ...
        const label = isPositive ? 'El mas' : 'El'
        await m.reply(`*${label} ${command} de aqui es* @${randomMember.split('@')[0]}`, { mentions: [randomMember] })
        m.react('✅')
    } catch (error) {
        m.react('☢')
