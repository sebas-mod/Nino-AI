const pluginConfig = {
    ...
    category: 'fun',
    description: 'Show de magia - expulsa a un miembro dramaticamente',
    usage: '.sulap',
    example: '.sulap',
    isOwner: false,
    isPremium: false,
...
const successLines = [
    '💨 *POOF!* Y... desaparecio!',
    '🌟 Magia exitosa! Hasta la proxima~',
    '✨ Te ausentas por ahora, esperamos la siguiente!',
    '🎪 Show terminado! 👏'
]
...
async function handler(m, { sock }) {
    ...
    const sent = await m.reply(`🎩✨ *ᴘᴇʀᴛᴜɴᴊᴜᴋᴀɴ sᴜʟᴀᴘ*\n\n` +
            `A quien quieres hacer desaparecer?\n\n` +
            `> Responde este mensaje + menciona a la persona`)
...
async function replyHandler(m, sock) {
    ...
    if (targetNumber === botNumber) {
        await sock.sendMessage(m.chat, { text: '🎭 El bot no puede hacerse desaparecer a si mismo!' })
        return true
    }

    if (targetJid === m.sender) {
        await sock.sendMessage(m.chat, { text: '🎭 No puedes hacerte desaparecer a ti mismo!' })
        return true
    }
    ...
    try {
        ...
        if (!target) {
            await sock.sendMessage(m.chat, { text: '👻 Esa persona no esta en el grupo!' })
            return true
        }
        ...
        await sock.sendMessage(m.chat, { text: `😅 La magia fallo...\n\n> ${error.message}` })
    }
