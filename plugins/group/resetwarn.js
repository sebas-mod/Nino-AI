...
const pluginConfig = {
    ...
    category: 'group',
    description: 'Restablece advertencias de miembros',
...
async function handler(m, { sock }) {
    ...
    if (!targetUser) {
        await m.reply(
            `⚠️ *ᴄᴀʀᴀ ᴘᴀᴋᴀɪ*\n\n` +
            `> Reply mensaje user + \`${m.prefix}resetwarn\`\n` +
    ...
    await m.reply(
        `✅ *ᴡᴀʀɴɪɴɢ ᴅɪʀᴇsᴇᴛ*\n` +
        `Warning @${targetName} correctamente direset!\n` +
        `Seaun nonya: *${prevCount}/${maxWarns}*\n` +
        `Sekarang: *0/${maxWarns}*`,
        { mentions: [targetUser] }
    )
}
