...
const pluginConfig = {
    ...
    category: 'group',
    description: 'Gestiona solicitudes de ingreso al grupo (aceptar/rechazar)',
    usage: '.acc <list|approve|reject> [all|numero]',
    ...
    try {
        ...
        for (const target of targets) {
            try {
                ...
                text += `${ok ? '✅' : '❌'} ${number} — ${ok ? label : 'Fallido'}\n`
                if (ok) successCount++
            } catch {
                const number = target.jid.split('@')[0]
                text += `❌ ${number} — Error\n`
        ...
        return m.reply(
            ...
            text + `\n` +
            `> ✅ ${successCount}/${targets.length} correctamente`
        )
    } catch (error) {
        await m.react('☢')
