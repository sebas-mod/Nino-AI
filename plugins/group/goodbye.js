...
const pluginConfig = {
  ...
  alias: ["bye", "leave"],
  category: "group",
  description: "Configura el mensaje de despedida del grupo",
...
async function buildGoodbyeMessage(
  ...
  const quotes = [
    ...
    `Terima kasih ya jadi bagian de grupo ini.`,
    `Espero que podamos encontrarnos de nuevo en otro momento.`,
...
Terima kasih ya pernah berjalan bersama.`,
    `💫 Sayonara~
No es el final, solo un hasta luego.
Semoga hari-harimu selalu hangat.`,
    `🌌 Minna-san...
Satu bintang berpindah langit malam ini.
Dose va a yang terbaik paranya ya.`,
  ];
  ...
  msg += `> 👥 *Quedan Miembro* : ${memberCount}\n`;
  ...
  try {
    ...
    if (goodbyeType === 2) {
      const cardBody = groupData?.goodbyeMsg
        ...
        : `Terima kasih ya bergabung di *${groupName}*\nQuedan ${memberCount} miembro`;
...
async function handler(m, { sock }) {
  ...
  if (sub === "on" && sub2 === "all") {
    ...
    try {
      ...
      return m.reply(
        `✅ *ɢᴏᴏᴅʙʏᴇ ɢʟᴏʙᴀʟ ᴏɴ*\n\n` +
          `> Goodbye activado en *${count}* grupo!`,
      );
    } catch (err) {
      m.react("☢");
  ...
  if (sub === "off" && sub2 === "all") {
    ...
    try {
      ...
      return m.reply(
        `❌ *ɢᴏᴏᴅʙʏᴇ ɢʟᴏʙᴀʟ ᴏꜰꜰ*\n\n` +
          `> Goodbye desactivado di *${count}* grupo!`,
      );
    } catch (err) {
      m.react("☢");
      return m.reply(te(m.prefix, m.command, m.pushName));
    ...
    if (currentStatus) {
      return m.reply(
        `⚠️ *ɢᴏᴏᴅʙʏᴇ ᴀʟʀᴇᴀᴅʏ ᴀᴄᴛɪᴠᴇ*\n\n` +
          `> Estado: *✅ ON*\n` +
          `> Goodbye ya activo di grupo ini.\n\n` +
          `_Usa \`${m.prefix}goodbye off\` para menonactivokan._`,
      );
    }
    db.setGroup(m.chat, { goodbye: true, leave: true });
    return m.reply(
      `✅ *ɢᴏᴏᴅʙʏᴇ ᴀᴋᴛɪꜰ*\n\n` +
        `> Goodbye message correctamente activado!\n` +
        `> Miembro yang keluar se va a diberi mensaje.\n\n` +
        `_Usa \`${m.prefix}setgoodbye\` para custom mensaje._`,
    );
  }
  if (sub === "off") {
    if (!currentStatus) {
      return m.reply(
        `⚠️ *ɢᴏᴏᴅʙʏᴇ ᴀʟʀᴇᴀᴅʏ ɪɴᴀᴄᴛɪᴠᴇ*\n\n` +
          `> Estado: *❌ OFF*\n` +
          `> Goodbye ya nonactivo di grupo ini.\n\n` +
          `_Usa \`${m.prefix}goodbye on\` para mengactivokan._`,
      );
    }
    db.setGroup(m.chat, { goodbye: false, leave: false });
    return m.reply(
      `❌ *ɢᴏᴏᴅʙʏᴇ ɴᴏɴᴀᴋᴛɪꜰ*\n\n` +
        `> Goodbye message correctamente desactivado.\n` +
        `> Miembro yang keluar tidak se va a diberi mensaje.`,
    );
  }
  m.reply(
    `👋 *ɢᴏᴏᴅʙʏᴇ sᴇᴛᴛɪɴɢs*\n\n` +
      ...
      `> \`${m.prefix}setgoodbye\` → Custom mensaje\n` +
