...
const pluginConfig = {
    ...
    category: 'group',
    description: 'Quita el silencio a un miembro especifico',
    usage: '.unmutemember <@tag/reply/numero>',
...
async function handler(m, { sock }) {
    ...
    if (m.command === 'listmutemember' || m.command === 'listmute') {
        if (mutedMembers.length === 0) {
            return m.reply(`🔇 *LIST MUTED MEMBERS*\n\n> No hay miembro yang dimute di grupo ini`)
        }
        ...
        txt += `╰┈┈⬡\n\n> Total: \`${mutedMembers.length}\` miembro dimute`
    ...
    if (!targetJid) {
        return m.reply(
            `🔊 *UNMUTE MEMBER*\n\n` +
            `> Quita el silencio a un miembro especifico\n\n` +
            `\`Ejemplo:\`\n` +
            `> ${m.prefix}unmutemember @user\n` +
            `> ${m.prefix}unmutemember 6281234567890\n` +
            `> Responde un mensaje del miembro + ${m.prefix}unmutemember`
        )
    }
    ...
    if (index === -1) {
        return m.reply(`❌ *ɢᴀɢᴀʟ*\n\n> Miembro @${targetNumber} tidak sedang dimute`, { mentions: [targetJid] })
    }
