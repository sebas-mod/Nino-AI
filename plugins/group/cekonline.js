...
const pluginConfig = {
    ...
    category: 'group',
    description: 'Consulta los miembros en linea del grupo',
    usage: '.cekonline',
    example: '.cekonline',
    isOwner: false,
    isPremium: false,
...
async function handler(m, { sock }) {
    ...
    try {
        ...
        if (participants.length === 0) {
            m.react('❌')
            return m.reply(`❌ *ɢᴀɢᴀʟ*\n\n> No se pudieron obtener datos de miembros del grupo`)
        }
        
        await m.reply(`🔍 *ᴍᴇɴᴄᴀʀɪ ᴍᴇᴍʙᴇʀ ᴏɴʟɪɴᴇ...*\n\n> Esperando respuesta de ${participants.length} miembro\n> Estimacion: 5-10 segundos`)
        ...
        text += `┃ 👤 ᴛᴏᴛᴀʟ: \`${participants.length}\` miembro\n`
        text += `┃ 🟢 ᴏɴʟɪɴᴇ: \`${onlineMembers.length}\` miembro\n`
        text += `╰┈┈⬡\n\n`
        
        if (onlineMembers.length === 0) {
            text += `> _No se detectaron miembros en linea_\n`
            text += `> _Pastikan miembro telah membuka WA_`
        } else {
            text += `╭┈┈⬡「 🟢 *ᴍᴇᴍʙᴇʀ ᴏɴʟɪɴᴇ* 」\n`
            ...
            for (const jid of onlineMembers) {
                if (count >= 50) {
                    text += `┃ ... dan ${onlineMembers.length - 50} miembro lainnya\n`
                    break
                }
