...
const pluginConfig = {
    ...
    category: 'group',
    description: 'Hidetag con soporte para responder mensajes (texto/media)',
    usage: '.htprem [mensaje] o reply mensaje',
    example: '.htprem o reply mensaje lalu .htprem',
    isOwner: false,
    isPremium: false,
    isGroup: true,
    isPrivate: false,
...
async function handler(m, { sock }) {
    try {
        ...
        if (!text) {
            return m.reply(
                `📢 *HIDETAG PREMIUM*\n\n` +
                `• Reply mensaje lalu ketik \`${m.prefix}ht\`\n` +
                `• Atau ketik \`${m.prefix}ht <custom tag> | <mensaje>\`\n\n` +
                `• Ejemplo: \`${m.prefix}ht everyone | hai semua\`\n\n` +
                `Support: teks, imagen, video, sticker, audio, dokumen`
            )
        }
