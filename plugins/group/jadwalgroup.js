...
const pluginConfig = {
    ...
    category: 'group',
    description: 'Programa apertura/cierre automatico del grupo',
...
「 📋 *sᴛᴀᴛᴜs* 」
🔓 ᴏᴘᴇɴ: *${openTime || 'Tidak activo'}*
🔒 ᴄʟᴏsᴇ: *${closeTime || 'Tidak activo'}*
...
\`.jadwalgroup hapus close\``;
    ...
    if (action === 'hapus' || action === 'delete' || action === 'remove') {
        ...
        if (type === 'open') {
            ...
            await m.reply(
                `✅ *ʙᴇʀʜᴀsɪʟ*\n\n` +
                `> Jadwal *buka grupo* otomatis telah eliminado.`
            );
        } else {
            ...
            await m.reply(
                `✅ *ʙᴇʀʜᴀsɪʟ*\n\n` +
                `> Jadwal *tutup grupo* otomatis telah eliminado.`
            );
        }
        return;
    }
...
> _Grupo se va a otomatis ${action === 'open' ? 'dibuka' : 'ditutup'}_
