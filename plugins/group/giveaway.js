  ...
  if (session.step === "q1") {
    ...
    try {
      ...
      await sock.sendButton(
        m.chat,
        null,
        "🎁 *GIVEAWAY CREATOR*\n\nPregunta 2/3:\nQuieres ejecutar el giveaway en este grupo u otro grupo?",
        m,
        { buttons, footer: "Elige grupo para giveaway" },
      );
    } catch (e) {
      session.groupId = currentGroup;
      session.step = "q3";
      await m.reply("⚠️ Fallido obtener lista grupo. Menggunse va a grupo ini.");
...
async function askPrizeDetails(m, sock, session) {
  try {
    ...
    await m.reply("⚠️ Fallido enviar PM. Silse va a chat bot dulu lalu ulangi.");
...
async function createGiveaway(session, sock, m) {
  ...
  const giveawayText =
    ...
    `┃ 🆔 ɪᴅ: \`${giveawayId}\`\n` +
    `╰┈┈⬡\n\n` +
    `> Klik tombol *Join* para ikut giveaway!`;
  ...
  try {
    const sentMsg = await sock.sendButton(
      ...
      {
        buttons: joinButton,
        footer: "Klik Join para berpartisipasi",
        contextInfo: getCtx(),
      },
    );
  ...
  await sock.sendMessage(
    session.adminJid,
    {
      text: "✅ Giveaway creado correctamente! Revisa el mensaje del giveaway en el grupo elegido.",
      contextInfo: getCtx(),
    },
    { quoted: m },
  );
  ...
  await sock.sendMessage(
    giveaway.chatId,
    {
      text:
        ...
        `╰┈┈⬡\n\n` +
        `> Hadiah dikirim ke private chat ganadores!`,
      ...
      await sock.sendMessage(
        winnerJid,
        {
          text:
            ...
            `╰┈┈⬡\n\n` +
            `╭┈┈⬡「 🎁 *ᴅᴇᴛᴀɪʟ ʜᴀᴅɪᴀʜ* 」\n` +
            `${giveaway.prizeDetails || "Hubungi admin para detail"}\n` +
            `╰┈┈⬡\n\n` +
            `> _Esta es informacion oficial del bot._`,
          contextInfo: ctx,
        },
...
const plugin = {
  ...
  alias: "ga",
  category: "group",
  description: "Sistema de sorteos con botones interactivos",
  admin: false,
};
...
async function handler(m, { sock }) {
  ...
  if (createCmds.includes(cmd)) {
    if (!m.isAdmin && !m.isOwner)
      return m.reply("⚠️ Solo admins pueden crear giveaways!");
    if (!m.isGroup) return m.reply("⚠️ Usalo en un grupo!");
    if (createSessions.has(m.sender))
      return m.reply("⚠️ Kamu masih punya sesi pembuatan activo!");
    ...
    await m.reply(
      "🎁 *GIVEAWAY CREATOR*\n\n" +
        ...
        "nama | durasi | jumlah ganadores\n\n" +
        "Ejemplo: Premium Account | 5m | 1\n" +
        "Durasi: 30s, 5m, 1h, 1d\n\n" +
        "> Bot se va a diam jika format salah",
    );
    return;
  }
  ...
  if (cmd === "giveaway" && args[0]?.toLowerCase() === "selectgroup") {
    ...
    await m.reply(
      "✅ Grupo elegido! Revisa el chat privado para la siguiente pregunta.",
    );
    await askPrizeDetails(m, sock, session);
    return;
  }
  ...
  if (cmd === "giveaway" && args[0]?.toLowerCase() === "join") {
    ...
    const giveaways = db.setting("giveaways") || {};
    const giveaway = giveaways[giveawayId];
    if (!giveaway) return m.reply("⚠️ Giveaway no se encontro!");
    if (giveaway.ended) return m.reply("⚠️ El giveaway ya termino!");
    if (giveaway.participants.includes(m.sender))
      return m.reply("⚠️ Ya te uniste!");
    ...
    await m.react("✅");
    await m.reply(
      `✅ @${m.sender.split("@")[0]} correctamente join giveaway! (${giveaway.participants.length} peserta)`,
    );
    return;
  }
  ...
  if (deleteCmds.includes(cmd)) {
    ...
    const giveaways = db.setting("giveaways") || {};
    if (!giveaways[giveawayId]) return m.reply("⚠️ Giveaway no se encontro!");

    delete giveaways[giveawayId];
    db.setting("giveaways", giveaways);
    await m.reply(`✅ Giveaway \`${giveawayId}\` se elimino correctamente!`);
    return;
  }
  ...
  if (rerollCmds.includes(cmd)) {
    ...
    const giveaway = giveaways[giveawayId];
    if (!giveaway) return m.reply("⚠️ Giveaway no se encontro!");
    if (!giveaway.ended) return m.reply("⚠️ Giveaway aun no berakhir!");
    if (giveaway.participants.length === 0)
  ...
  if (cmd === "giveaway") {
    await m.reply(
      "🎁 *GIVEAWAY MENU*\n\n" +
        ...
        `┃ ${prefix}giveawayreroll — Reroll ganadores\n\n` +
