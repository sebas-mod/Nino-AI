...
const pluginConfig = {
  ...
  category: "group",
  description: "Muestra la lista de advertencias de miembros",
...
async function handler(m, { sock }) {
  ...
  if (targetUser) {
    ...
    if (usersWithWarnings.length === 0) {
      await m.reply(`✅ No hay miembro con warning di grupo ini.`);
      return;
    }
    ...
    txt += `\n> Escribe \`${m.prefix}listwarn @user\` para detail`;
