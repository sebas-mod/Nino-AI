...
const pluginConfig = {
    ...
    category: 'group',
    description: 'Configura antitoxic en el grupo',
...
function gpMsg(key, replacements = {}) {
    const defaults = {
        antitoxicWarn: '⚠ @%user% uso palabras ofensivas.\nAdvertencia %warn% de %max%; la proxima infraccion puede terminar en %method%.',
...
async function handler(m, { sock }) {
    ...
    if (subCommand === 'warn') {
        ...
        if (!count || count < 1 || count > 10) {
            return m.reply(`❌ Ingresa angka 1-10\n> Ejemplo: \`.antitoxic warn 5\``)
        }
        db.setGroup(m.chat, { toxicMaxWarn: count })
        m.react('✅')
        await m.reply(`✅ Maximo de advertencias cambiado a *${count}*`)
        return
    }
    ...
    if (subCommand === 'metode' || subCommand === 'method' || subCommand === 'mode') {
        ...
        if (!method || !['kick', 'delete'].includes(method)) {
            return m.reply(`❌ Elige metode: *kick* o *delete*\n> Ejemplo: \`.antitoxic metode kick\``)
        }
        db.setGroup(m.chat, { toxicMethod: method })
        m.react('✅')
        await m.reply(`✅ Metodo cambiado a *${method}*`)
        return
    }

    await m.reply(`❌ Subcomando desconocido.\n> Escribe \`.antitoxic\` para ver la lista comando.`)
}
