...
const pluginConfig = {
    ...
    alias: ['away', 'brb'],
    category: 'group',
    description: 'Establece el estado AFK con motivo',
    usage: '.afk <alasan>',
    example: '.afk lagi mse va a',
    isOwner: false,
    isPremium: false,
    isGroup: false,
    isPrivate: false,
...
function formatDuration(ms) {
    ...
    if (hours > 0) {
        ...
        return `${minutes} menit ${seconds % 60} segundos`
    } else {
...
async function handler(m, { sock }) {
    const reason = m.text || 'No hay alasan'
    setAfkUser(m.sender, reason)
    await m.reply(
        `💤 *ᴀꜰᴋ ᴀᴋᴛɪꜰ*\n\n` +
        `\`\`\`@${m.sender.split('@')[0]} sekarang AFK\`\`\`\n` +
        `🍀 \`Alasan:\` *${reason}*\n\n` +
        `_Escribe apapun para menonactivokan AFK._`,
        { mentions: [m.sender] }
    )
}
        ...
        for (const mentioned of m.mentionedJid) {
            ...
            if (mentionedAfk) {
                ...
                await m.reply(`💤 *ᴜsᴇʀ ᴀꜰᴋ*\n\n` +
                        `\`\`\`Shh, no lo molestes!\`\`\` \`@${mentioned.split('@')[0]}\` esta AFK\n` +
                        `🍀 \`Alasan:\` *${mentionedAfk.reason}*\n` +
                        `🍀 \`Sejak:\` *${duration} atras*`, { mentions: [mentioned] })
            }
        }
    }
}
