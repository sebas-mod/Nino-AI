...
const pluginConfig = {
    ...
    category: 'group',
    description: 'Agrega miembros al grupo (admite multiples)',
    usage: '.add <numero1> [numero2] [numero3]... [link_grupo]',
...
async function handler(m, { sock }) {
    ...
    if (args.length === 0) {
        return m.reply(
            `👥 *ᴀᴅᴅ ᴍᴇᴍʙᴇʀ*\n\n` +
            `> Como usar:\n` +
            `> 1. En el grupo: \`${m.prefix}add <numero>\`\n` +
            `> 2. Multiple: \`${m.prefix}add <numero1> <numero2> ...\`\n` +
            `> 3. En privado: \`${m.prefix}add <numero> <link_grupo>\`\n\n` +
            `> Ejemplo:\n` +
            `> \`${m.prefix}add 6281234567890\`\n` +
            `> \`${m.prefix}add 628123 628456 628789\`\n` +
            `> \`${m.prefix}add 628123 https://chat.whatsapp.com/xxx\`\n\n` +
            `> Requisitos:\n` +
            `> - El bot debe ser admin en el grupo objetivo\n` +
            `> - Quien ejecute el comando debe ser admin`
        )
    }
    ...
    for (const arg of args) {
        ...
        if (linkMatch) {
            try {
                ...
                return m.reply(`❌ *ɢᴀɢᴀʟ*\n\n> El enlace del grupo no es valido o ya expiro!`)
            }
    ...
    if (!targetGroup) {
        return m.reply(`❌ *ɢᴀɢᴀʟ*\n\n> Ejecutalo en un grupo o incluye el enlace del grupo!\n\n\`${m.prefix}add <numero> <link_grupo>\``)
    }
    ...
    try {
        ...
        if (validNumbers.length === 0) {
            return m.reply(`❌ *ɢᴀɢᴀʟ*\n\n> Todos los numeros ya estan en el grupo!`)
        }
        ...
        let resultText = `🥗 @${m.sender.split('@')[0]} agrego miembros al grupo\n\n`
        
        if (successList.length > 0) {
            resultText += `Ada *${successList.length}* miembros agregados correctamente:\n`
            successList.forEach(n => resultText += `• @${n}\n`)
            resultText += `\n`
        }
        
        if (invitedList.length > 0) {
            resultText += `📨 *Dan ada juga *${invitedList.length}* miembros invitados:*\n`
