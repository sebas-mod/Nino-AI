...
const pluginConfig = {
    ...
    category: 'group',
    description: 'Da una advertencia a un miembro',
...
async function handler(m, { sock }) {
    ...
    if (!args[0] && !m.quoted && (!m.mentionedJid || m.mentionedJid.length === 0)) {
        return m.reply(
            ...
            `Sistem manajemen pelanggaran para miembro grupo.\n` +
            `Batas Warning: *${maxWarns} kali* (Otomatis Kick)\n\n` +
            `*PENGGUNAAN:*\n` +
            `• *${m.prefix}warn @user <alasan>* — Miembroi warning\n` +
            `• *${m.prefix}warn max <angka>* — Mengubah batas maksimal warning\n` +
            `• *${m.prefix}listwarn* — Melihat lista miembro bermasalah\n` +
            `• *${m.prefix}resetwarn @user* — Menghapus semua warning miembro\n\n` +
            `*PENJELASAN ALUR PENGGUNAAN:*\n` +
            `1. Saat miembro melakukan pelanggaran pertama, beri mereka SP1: *${m.prefix}warn @user Spam mensaje*\n` +
            `2. Bot se va a mencatat "Spam mensaje" sebagai warning ke-1 mereka.\n` +
            `3. Jika melanggar lagi, beri peringatan kedua con alasan baru: *${m.prefix}warn @user Berkata kasar*\n` +
            `4. Jika total peringatan miembro mencapai batas maksimal (saat ini *${maxWarns}*), bot se va a otomatis MENGELUARKAN (Kick) miembro tersebut.\n` +
            `5. Riwayat pelanggaran bisa dilihat lengkap con mengetik *${m.prefix}listwarn @user*.`
        )
    }
    if (args[0]?.toLowerCase() === 'max') {
        ...
        return m.reply(`✅ *BATAS WARNING DIUBAH*\n\nMaksimal warning grupo ini telah diupdate menjadi *${newMax} kali*.`)
    }
    ...
    if (!targetUser) {
        await m.reply(
            `⚠️ *CARA PAKAI*\n\n` +
            `> Reply mensaje user + \`${m.prefix}warn alasan\`\n` +
    ...
    try {
        ...
        if (participant?.admin) {
            await m.reply(`❌ No se puede miembroikan warning kepada admin grupo.`)
            return
        }
    } catch (e) {}
    ...
    const reason = reasonArg || 'No hay alasan'
    ...
    if (warnCount >= maxWarns) {
        try {
            ...
            await m.reply(
                ...
                `@${targetName} telah dikeluarkan de grupo karena mencapai batas pelanggaran!\n\n` +
                `*Rincian:*\n` +
