const pluginConfig = {
    ...
    category: 'group',
    description: 'Cambia la descripcion del grupo',
    usage: '.setdeskgc <deskripsi baru>',
    example: '.setdeskgc Grupo para diskusi',
    isOwner: false,
    isPremium: false,
    isGroup: true,
    isPrivate: false,
...
async function handler(m, { sock }) {
    ...
    try {
        ...
        if (descToSet) {
            await m.reply(
                `✅ Deskripsi grupo correctamente diperbarui!`
            )
        } else {
            await m.reply(
                `✅ Deskripsi grupo se elimino correctamente!`
            )
        }
    } catch (error) {
        await m.reply(
            `❌ *ɢᴀɢᴀʟ*\n\n` +
            `> Tidak dapat mengubah deskripsi grupo.\n` +
            `> _${error.message}_`
        )
    }
}
