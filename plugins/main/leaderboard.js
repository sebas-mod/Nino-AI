...
const pluginConfig = {
    ...
    category: 'main',
    description: 'Ver ranking global (koin, exp, energia)',
    usage: '.leaderboard',
    example: '.topkoin',
    isOwner: false,
    isPremium: false,
...
async function handler(m, { sock }) {
    ...
    if (users.length === 0) {
        return m.reply(`📊 *ʟᴇᴀᴅᴇʀʙᴏᴀʀᴅ*\n\n> Aun no hay usuarios registrados en la base de datos.`)
    }
    ...
    if (type === 'overview') {
        ...
        const overviewText = `🏆 *RESUMEN DEL RANKING* 🏆\n\n` +
            `_Elige un boton de abajo para ver el ranking!_`
            try {
    ...
    let title, emoji, field, formatValor
    
    if (type === 'koin') {
        title = 'TOP GLOBAL KOIN'
        emoji = '💰'
        field = 'koin'
        formatValor = (u) => `Rp ${formatNumber(u.koin)}`
    } else if (type === 'exp') {
        title = 'TOP GLOBAL LEVEL'
        emoji = '✨'
        field = 'exp'
        formatValor = (u) => `Lv. ${u.level} (${formatNumber(u.exp)} XP)`
    } else if (type === 'energi') {
        title = 'TOP GLOBAL ENERGI'
        emoji = '⚡'
        field = 'energi'
        formatValor = (u) => `${formatNumber(u.energi)} Energi`
    }
    ...
    let text = `🏆 *${title}* 🏆\n\n`
    text += `Ranking actual de los usuarios mas altos!\n\n`
    text += `╭┈┈⬡「 ${emoji} *RANKING* 」\n`
    ...
    top10.forEach((u, i) => {
        ...
        const isMe = u.jid === senderJid ? " *(Tu)*" : ""
        
        text += `┃ ${medal} @${u.jid.split('@')[0]}${isMe}\n`
        text += `┃    └ ${formatValor(u)} (${pct}%)\n`
    ...
    if (myRankIndex !== -1) {
        text += `> Tu posicion: *#${myRankIndex + 1}* de *${formatNumber(users.length)}* usuarios.`
    } else {
        text += `> Aun no estas registrado en la base de datos.`
    }
