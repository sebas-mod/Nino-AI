...
const pluginConfig = {
  ...
  category: "main",
  description: "Informacion de donaciones para apoyar el bot con QRIS",
  usage: ".donasi",
  example: ".donasi",
  isOwner: false,
  isPremium: false,
...
async function handler(m, { sock }) {
  ...
  const benefits = donasiConfig.benefits || [
    "Apoya el desarrollo",
    "Servidor mas estable",
    "Nuevas funciones mas rapido",
    "Soporte prioritario",
  ];

  let text = `DONACION AL OWNER ${botName} 🙏`;
  ...
  if (payments.length > 0 || links.length > 0) {
    ...
    for (const pay of payments) {
      text += `🏦 *${pay.name
        ...
        .join("")}*\n`;
      text += `◦ ${pay.number} (a nombre de ${pay.holder})\n`;
    }
  ...
  text += `_Cualquier donacion es muy valiosa_\n`;
