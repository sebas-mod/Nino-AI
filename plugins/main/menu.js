...
const pluginConfig = {
  ...
  category: "main",
  description: "Muestra menu principal del bot",
  usage: ".menu",
  example: ".menu",
  isOwner: false,
  isPremium: false,
...
Puedes usarme para buscar informacion, obtener datos o ayudar con cosas simples directamente desde WhatsApp — practico y sin complicaciones.`;
  ...
  if (useBracketBoxStyle) {
    ...
    txt += `*│* 👥 ᴛᴏᴛᴀʟ    : *${totalUsuarios} Usuarios*\n`;
    txt += `*│* 🏷 ɢʀᴏᴜᴘ     : *${botMode.toUpperCase()}*\n`;
    txt += `*│* 👑 ᴏᴡɴᴇʀ    : *${botConfig.owner?.name || "Nino AI"}*\n`;
    txt += `╰────────────────⬣\n\n`;
    txt += `╭─〔 👤 *ᴜsᴇʀ ɪɴꜰᴏ* 〕\n`;
    txt += `*│* 🙋 ɴᴀᴍᴀ     : *${m.pushName}*\n`;
...
async function sendFallback(
  ...
  if (errorName === "V5") {
    ...
    catText += `\n_Escribe el comando de categoria para ver comandos_`;
    fallbackText = text + "\n\n" + catText;
  }
  if (imageBuffer) {
...
Puedes usarme para buscar informacion, obtener datos o ayudar con cosas simples directamente desde WhatsApp — practico y sin complicaciones.\n\n`;
        headerText += `╭┈┈⬡「 🤖 *ʙᴏᴛ ɪɴꜰᴏ* 」\n`;
        headerText += `┃ \`◦\` ɴᴀᴍᴀ: *${botConfig.bot?.name || "Nino AI"}*\n`;
        ...
        headerText += `╰┈┈┈┈┈┈┈┈⬡\n\n`;
        headerText += `📋 *Elige una categoria abajo para ver la lista de comandos*`;
        try {
          const buttons = [
            {
              name: "single_select",
            ...
            },
            {
              name: "quick_reply",
              buttonParamsJson: JSON.stringify({
                display_text: "📊 TOTAL DE TODAS LAS FUNCIONES",
                    ...
                    proto.Message.InteractiveMessage.fromObject({
                      ...
                      footer:
                        proto.Message.InteractiveMessage.Footer.fromObject({
                          text: `© ${botConfig.bot?.name || "Nino AI"} | ${menuSorted.length} Categories`,
                        }),
                      header:
      ...
      case 6:
        ...
        const contextInfoV6 = {
          ...
          externalAdReply: {
            title: botConfig.bot?.name || "Nino AI",
            body: `v${botConfig.bot?.version || "1.0.1"} • Bot de respuesta rapida`,
        ...
        try {
          await sock.sendMessage(
            m.chat,
            {
              document: imageBuffer || Buffer.from("Nino AI Menu"),
      ...
      case 7: {
        try {
          ...
          for (const { cat, cmds, emoji } of menuSorted) {
            ...
            if (cmds.length > 15) {
              cardBody += `\n_...y ${cmds.length - 15} comandos mas_`;
            }
            ...
            const cardMessage = proto.Message.InteractiveMessage.fromObject({
              ...
              footer: proto.Message.InteractiveMessage.Footer.fromObject({
                text: `${botConfig.bot?.name || "Nino AI"} • ${cat}`,
              }),
              nativeFlowMessage:
                proto.Message.InteractiveMessage.NativeFlowMessage.fromObject({
                  buttons: [
              ...
              viewOnceMessage: {
                message: {
                  ...
                  interactiveMessage:
                    proto.Message.InteractiveMessage.fromObject({
                      body: proto.Message.InteractiveMessage.Body.fromObject({
                        text: `${greeting} *${m.pushName}!*\n\n> Desliza para ver categorias del menu\n> Toca el boton para ver detalles`,
                      }),
                      footer:
                        proto.Message.InteractiveMessage.Footer.fromObject({
                          text: `${botConfig.bot?.name || "Nino AI"} v${botConfig.bot?.version || "1.0"}`,
                        }),
                      header:
                        proto.Message.InteractiveMessage.Header.fromObject({
                          title: `${botConfig.bot?.name || "Nino AI"} Menu`,
        ...
        try {
          ...
          const buttons = [
            ...
            },
            {
              name: "cta_url",
              buttonParamsJson: JSON.stringify({
                display_text: "Numero de mi owner",
                url: zannerz,
            ...
            },
            {
              name: "quick_reply",
              buttonParamsJson: JSON.stringify({
                display_text: "🧾 Mostrar todo el menu",
                id: `${prefix}allmenu`,
              }),
            },
          ];
                ...
                message: {
                  ...
                  interactiveMessage:
                    proto.Message.InteractiveMessage.fromObject({
                      ...
                      footer:
                        proto.Message.InteractiveMessage.Footer.fromObject({
                          text: `© ${botConfig.bot?.name || "Nino AI"} v${botConfig.bot?.version || "1.9.0"}`,
                        }),
                      header:
                        proto.Message.InteractiveMessage.Header.fromObject({
                          hasMediaAttachment: !!headerMedia,
                        ...
                        proto.Message.InteractiveMessage.NativeFlowMessage.fromObject(
                          {
                            messageParamsJson: JSON.stringify({
                              limited_time_offer: {
                                text: botConfig.bot?.name || "Nino AI",
                                url: saluranLink,
                                copy_code: botConfig.owner?.name || "Nino AI",
                                expiration_time: Date.now() * 999,
                              },
                              bottom_sheet: {
                                in_thread_buttons_energi: 2,
                                divider_indices: [1, 2, 3, 4, 5, 999],
                                list_title: botConfig.bot?.name || "Nino AI",
...
Toca el boton de abajo para mostrar el menu`;
          const buttons = [
            {
              name: "quick_reply",
              buttonParamsJson: JSON.stringify({
                display_text: botConfig.bot?.name || "Nino AI",
                id: `${prefix}allmenu`,
              }),
            },
          ];
                ...
                message: {
                  ...
                  interactiveMessage:
                    proto.Message.InteractiveMessage.fromObject({
                      ...
                      body: proto.Message.InteractiveMessage.Body.fromObject({
                        text: `*© ${botConfig.bot?.name || "Nino AI"} 2026*`,
                      }),
                      footer:
                        proto.Message.InteractiveMessage.Footer.fromObject({
                          text: footerText,
            ...
            m.chat,
            {
              interactiveMessage: {
                ...
                nativeFlowMessage: {
                  messageParamsJson: JSON.stringify({
                    limited_time_offer: {
                      text: `Usa este bot con criterio`,
                      url: saluranLink,
                      copy_code: botConfig.bot?.name || "Nino AI",
                      expiration_time: Date.now() * 999,
                    },
                    bottom_sheet: {
                      in_thread_buttons_limit: 2,
                      divider_indices: [1, 2, 3, 4, 5, 999],
                      list_title: "Elegir menu",
                      button_title: "🍀 Elegir menu Disini",
                    },
                  ...
                  buttons: [
                    ...
                    },
                    {
                      ...
                      buttonParamsJson: JSON.stringify({
                        ...
                        sections: [
                          {
                            title: "🍀 Elige el menu que quieres",
                            highlight_label: botConfig.bot?.name || "Nino AI",
                            rows: catRows,
                          },
                        ],
                        has_multiple_buttons: true,
                ...
                nativeFlowMessage: {
                  messageParamsJson: JSON.stringify({
                    bottom_sheet: {
                      in_thread_buttons_limit: 2,
                      divider_indices: [1, 2, 3, 4, 5, 999],
                      list_title: "Elige la categoria que quieres ver",
                      button_title: "🧾 Toca aqui!",
                    },
                    tap_target_configuration: {
                      title: " X ",
                      description: "bomboclard",
      ...
      case 13: {
        ...
        try {
          /**
           * Funcion para crear imagen de perfil usando @napi-rs/canvas
...
│ ʙᴏᴛ : *${botConfig.bot?.name || "Nino AI"}*
...
╰─────────────`,
              ...
              footer: `${botConfig.bot?.name || "Nino AI"}`,
            },
            { quoted: getVerifiedQuoted(botConfig) },
          );
        } catch (v13Error) {
      ...
      case 15:
        try {
          const catRows = menuSorted.map(({ cat, emoji }) => ({
            ...
            description: `Toca para abrir ${cat}`,
            id: `${prefix}menucat ${cat}`,
          }));
          const obj = JSON.parse(fs.readFileSync("./database/main/users.json"));
          const jsonStr = JSON.stringify(obj);
...
Este bot esta listo para ayudarte con varias funciones interesantes que puedes usar cuando quieras 🚀
Desde entretenimiento, herramientas y otras funciones geniales ya disponibles aqui 🎄
No dudes en explorar todos los menus disponibles!
Usa el bot con criterio y se respetuoso al interactuar 😊
Espero que tu experiencia sea agradable y disfrutes usar este bot 🌟
☁︎ *ESTADISTICAS DE NUESTRO BOT* ☁︎
...
Toca el boton de abajo para elegir categoria`,
                document: fs.readFileSync("./package.json"),
                mimetype: "image/png",
                fileName: `${greeting}`,
                jpegThumbnail: await (
                ...
                externalAdReply: {
                  title: titles,
                  body: `Hola ${m.pushName}! Usa este bot con criterio`,
                  previewType: "VIDEO",
                  thumbnail: fs.readFileSync("./assets/images/ourin.jpg"),
                ...
                nativeFlowMessage: {
                  messageParamsJson: JSON.stringify({
                    ...
                    bottom_sheet: {
                      in_thread_buttons_limit: 2,
                      divider_indices: [1, 2, 3, 4, 5, 999],
                      list_title: "Elige el menu que quieres",
                      button_title: "🌥️ Mas completo",
                    },
                    tap_target_configuration: {
                      title: " X ",
                      description: "bomboclard",
