...
async function loadAllPlugins() {
        ...
        try {
          ...
          if (plugin.config && plugin.config.name) {
            plugins.push({
              ...
              description: plugin.config.description || "Sin descripcion",
  ...
  try {
    ...
    const caseDescriptions = {
      cping: "Revisar ping case system",
  ...
  try {
    ...
    for (const plugin of allPlugins) {
      ...
      if (matchesKeyword(plugin.category, keyword)) {
        ...
        matchReason = matchReason || "categoria";
      }
      if (isMatch) {
    ...
    if (matches.length === 0) {
      m.react("❌");
      return m.reply(
        `🔍 *ʜᴀsɪʟ ᴘᴇɴᴄᴀʀɪᴀɴ*\n\n> No se encontro ninguna funcion con la palabra clave \`${keyword}\``,
      );
    }
    const saluranId = config.saluran?.id || "120363400911374213@newsletter";
    const saluranName = config.saluran?.name || config.bot?.name || "Nino AI";
    let text = `🔍 *ʜᴀsɪʟ ᴘᴇɴᴄᴀʀɪᴀɴ: "${keyword}"*\n`;
    text += `> Encontradas *${matches.length}* funciones\n`;
    text += `> Elige uno de los comandos de abajo:\n\n`;
