...
async function handler(m, { sock, db }) {
  ...
  if (!categoryArg) {
    ...
    let txt = `📂 *${toMonoUpperBold("LISTA DE CATEGORIAS")}*\n\n`;
    txt += `> Escribe \`${prefix}menucat <categoria>\`\n\n`;
    const categoryOrder = [
      "owner",
      "main",
      "utility",
    ...
    for (const cat of visibleCats) {
      ...
      txt += ` │ ${emoji} ${cat.toUpperCase()} │ \`${totalCmds}\` comandos\n`;
    }
    txt += `╰───────⬣\n\n`;
          ...
          return await sock.sendMessage(
            m.chat,
            {
              ...
              contextInfo: buildFullContextInfo(
                "Lista de categorias",
                `${visibleCats.length} categorias disponibles`,
              ),
            },
            { quoted: m },
          );
          ...
          const buttons = [
            {
              ...
              buttonParamsJson: JSON.stringify({
                ...
                sections: [{ title: "📋 ELIGE CATEGORIA", rows: catRows }],
              }),
            },
            {
              name: "quick_reply",
                    ...
                    proto.Message.InteractiveMessage.fromObject({
                      ...
                      header:
                        proto.Message.InteractiveMessage.Header.fromObject({
                          ...
                          subtitle: `${visibleCats.length} categorias`,
  ...
  if (!matchedCat) {
    return m.reply(
      `❌ *CATEGORIA NO ENCONTRADA*\n\n> Categoria \`${categoryArg}\` no existe.\n> Escribe \`${prefix}menucat\` para listar categorias.`,
    );
  }
  if (matchedCat === "owner" && !m.isOwner) {
    return m.reply(`❌ *ACCESO DENEGADO*\n\n> Esta categoria es solo para owner.`);
  }
  ...
  if (allCommands.length === 0) {
    return m.reply(
      `❌ *VACIO*\n\n> Categoria \`${matchedCat}\` no tiene comandos.`,
    );
  }
                ...
                interactiveMessage: proto.Message.InteractiveMessage.fromObject(
                  {
                    ...
                    header: proto.Message.InteractiveMessage.Header.fromObject({
                      ...
                      subtitle: `${allCommands.length} comandos`,
                      hasMediaAttachment: false,
                    }),
                    nativeFlowMessage:
                      proto.Message.InteractiveMessage.NativeFlowMessage.fromObject(
