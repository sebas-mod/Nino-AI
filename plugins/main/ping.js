...
const pluginConfig = {
  ...
  category: "main",
  description: "Revisar rendimiento y estado del sistema del bot en tiempo real",
  usage: ".ping",
  example: ".ping",
  isOwner: false,
  isPremium: false,
...
async function render(s, pf) {
  ...
  sRow(dx, ry, "Usuarios", s.dbUsuarios, "#e2e8f0", 240);
  ry += 16;
...
async function handler(m, { sock }) {
  ...
  try {
    ...
    const caption =
      ...
      `│  ◦ Libre: *${fmtSize(s.diskTotal - s.diskUsed)}* (${diskPct}%)\n` +
      `╰───────⬣\n\n` +
      `╭─〔 🌐 *ɴᴇᴛᴡᴏʀᴋ* 〕───⬣\n` +
      `│  ◦ ${s.networkInterface}\n` +
