...
const BASE_URL =
  "https://raw.githubusercontent.com/Leoo7z/Image-Source/main/image";
...
const pluginConfig = {
  ...
  category: "random",
  description: "Imagen aleatoria de anime/wallpaper (fuente Leoo7z)",
  usage: ".<nombre> (mira la lista de abajo)",
  example: ".naruto",
  isOwner: false,
  isPremium: false,
  isGroup: false,
...
async function getImages(source) {
  ...
  try {
    const response = await axios.get(`${BASE_URL}/${source}.json`, {
      timeout: 15000,
    });
...
async function handler(m, { sock }) {
  ...
  try {
    ...
    if (!images || images.length === 0) {
      return m.reply(`❌ No se pudieron obtener los datos de ${cmd}.`);
    }
    ...
    if (cmd === "shortquote") {
      ...
      await sock.sendMessage(
        ...
        {
          image: { url: imageUrl },
          caption: `🖼️ *Aleatorio ${cmd.charAt(0).toUpperCase() + cmd.slice(1)}*\n> ${randomIndex + 1}/${images.length}`,
        },
        { quoted: m },
      );
    }
