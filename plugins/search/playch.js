...
const pluginConfig = {
  ...
  category: "search",
  description: "Enviar música al canal (convertir a opus)",
...
async function getPlayChAudioDownload(url) {
  ...
  throw new Error(fallback?.mess || "No se pudo obtener la URL del audio del canal");
}
...
async function handler(m, { sock }) {
  ...
  if (!chId)
    return m.reply(
      `❌ El canal aún no está configurado. Usa \`--idch <id>\` o configúralo en config.js`,
    );
  ...
  try {
    ...
    info += `📡 Canal: \`${chId}\`\n`;
    info += `🔗 ${video.url}\n\n`;
    info += `_⏳ Enviando audio al canal, espera un momento..._`;
    ...
    m.react("✅");
    m.reply(`✅ *${title}* se envió correctamente al canal`);
  } catch (e) {
