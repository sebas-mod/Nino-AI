...
const pluginConfig = {
    ...
    category: 'tools',
    description: 'Consulta el ID y la información completa del canal desde un enlace',
...
async function handler(m, { sock }) {
    ...
    if (!text) {
        return m.reply(
            `── .✦ 𝗖𝗘𝗞 𝗜𝗗 𝗖𝗛𝗔𝗡𝗡𝗘𝗟 ✦. ── 𝜗ৎ\n\n` +
            `> Ingresa el enlace del canal de WhatsApp\n\n` +
            `> \`${m.prefix}cekidch https://whatsapp.com/channel/xxxxx\``
        )
    }

    if (!text.includes('https://whatsapp.com/channel/')) {
        return m.reply(`── .✦ ──\n\n> El enlace del canal no es válido .☘︎ ݁˖`)
    }
    ...
    try {
        ...
        const chVerificado = metadata.verification === 'VERIFIED' ? '✓ Verificado' : 'No verificado'
