...
const pluginConfig = {
  ...
  category: "tools",
  description: "Captura de sitio web en 3 versiones (desktop, móvil, tablet)",
...
async function handler(m, { sock }) {
  ...
  try {
    const data = await ourinApi.apiFaa.ssweb3hasil(targetUrl, {
      timeout: 60000,
    });

    if (!data?.status || !data?.results) {
      throw new Error("No se pudo tomar la captura");
    }
