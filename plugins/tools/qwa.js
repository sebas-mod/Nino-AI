...
const pluginConfig = {
    ...
    category: 'tools',
    description: 'Crea una imagen de cita de WhatsApp',
    usage: '.qwa [texto]',
...
async function handler(m, { sock }) {
    try {
        ...
        if (!textToQuote && !mainMsg.isMedia) {
            return m.reply(`❌ *FORMATO INCORRECTO*\n\nEnvía el comando \`.qwa <texto>\` o responde al mensaje de otra persona con \`.qwa\`.`)
        }
        await m.react('🕕')
        ...
        await sock.sendMessage(m.chat, {
            ...
            caption: `✅ Cita de WhatsApp creada correctamente!`
        }, { quoted: m })
