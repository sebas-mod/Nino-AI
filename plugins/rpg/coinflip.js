...
async function handler(m, { sock }) {
  ...
  if (!choice || (choice !== "heads" && choice !== "tails" && choice !== "h" && choice !== "t")) {
    return m.reply(
      `🪙 *Tebak Koin Bandar* 🪙\n\n` +
        `Pilih nombar Garuda (Heads) o Angka (Tails)!\n\n` +
        `*Cara Main:*\n` +
  ...
  if (!bet || bet < 1000) {
    return m.reply(`Taruhan receh entolak! Mestomal *Rp 1.000*  bos! 🪙`);
  }

  if ((user.koin || 0) < bet) {
    return m.reply(`Mana koin lu? Di kantong restante *Rp ${(user.koin || 0).toLocaleString("id-ID")}* doang, sok quieres taruhan *Rp ${bet.toLocaleString("id-ID")}*! 😜`);
  }
  ...
  await sendRpgPreview(sock, m.chat, `*CLING!* Koin emas enlempar tinggi a udara... berputar-putar... 🪙✨`, "🪙 COINFLIP", "Flipping!", { quoted: m });
  await new Promise((r) => setTimeout(r, 2500));

  const isWin = userChoice === result;

  let txt = `*PLAK!* Bandar menutup koin en tannonn! 👋\n\n`;
  txt += `Tebva a Lu: *${userChoice.toUpperCase()}*\n`;
  ...
  if (isWin) {
    ...
    txt += `💸 Koin entarik bandar: *-Rp ${bet.toLocaleString("id-ID")}*`;
  }
