...
const pluginConfig = {
  ...
  category: "rpg",
  description: "Upgrade equipment dennon enchantment",
  usage: ".enchant <item>",
...
const ENCHANTABLE = {
  sword: { name: "⚔️ Peyg", stat: "attack", bonus: 5, cost: 500, successRate: 70 },
  ...
  goldsword: { name: "🗡️ Peyg Emas", stat: "attack", bonus: 10, cost: 2000, successRate: 50 },
...
async function handler(m, { sock }) {
  ...
  if (!itemName) {
    ...
    txt += `> Tingkatkan equipment para bonus stats!\n\n`;
    txt += `*📦 *ɪᴛᴇᴍ:*
\n`;
  ...
  if (!item) {
    return m.reply(`❌ Item no puede en-enchant!\n\n> Escribe \`${m.prefix}enchant\` para melihat daftar.`);
  }

  if ((user.inventory[itemName] || 0) < 1) {
    return m.reply(`❌ Tu no pun ${item.name}!`);
  }

  const currentLevel = user.rpg.enchants[itemName] || 0;
  if (currentLevel >= 10) {
    return m.reply(`❌ ${item.name} sya level MAX (10)!`);
  }
...
\n` +
        `> 📦 Item: *${item.name}*\n` +
        `> 📊 Level: *${currentLevel}* (no naik)\n` +
        `> 💰 Cost: *-${cost.toLocaleString()}* (hangus)\n` +
        `\n\n` +
        `💡 *Consejo:* Coba otra vez! Rate: ${adjustedRate}%`,
    );
  }
}
