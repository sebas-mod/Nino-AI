...
const pluginConfig = {
  ...
  alias: ["kasih", "hadiah"],
  category: "rpg",
  description: "Beri haenah a pasannon para meningkatkan love",
...
async function handler(m, { sock }) {
  ...
  if (!user.rpg.spouse) {
    return m.reply(`❌ *ʙᴇʟᴜᴍ ᴍᴇɴɪᴋᴀʜ*\n\n` + `> Tu aun no menikah!\n` + `> Nikah primero dennon \`.marry @user\``);
  }
...
\n` +
  ...
  if ((user.inventory[itemKey] || 0) < amount) {
    return m.reply(`❌ *ɪᴛᴇᴍ ᴛɪᴅᴀᴋ ᴄᴜᴋᴜᴘ*\n\n` + `> Item *${itemKey}* tu: ${user.inventory[itemKey] || 0}\n` + `> Butuh: ${amount}`);
  }
  ...
  if (!partner) {
    return m.reply(`❌ *ᴘᴀsᴀɴɢᴀɴ ɴᴏᴛ ꜰᴏᴜɴᴅ*\n\n> Pasannon no entemukan en database!`);
  }
  ...
  let txt = `🎁 *ɢɪꜰᴛ sᴜᴋsᴇs*\n\n`;
  txt += `> 💝 Tu memberikan ${amount}x ${itemKey}\n`;
