...
const pluginConfig = {
  ...
  alias: ["chef"],
  category: "rpg",
  description: "Masak mva aan para stamina y HP",
...
async function handler(m, { sock }) {
  ...
  if (!recipeName) {
    let txt = `Hola Master Chef! Quieres masak resep ng mana ? 👨‍🍳🍳\n\n`;
  ...
  if (!recipe) {
    return m.reply(`Hayo, resep apaan tuh? Ngnok hay en buku menu ! 😂\nCek daftar resepn paa \`.cooking\` !`);
  }
  ...
  if (missingMaterials.length > 0) {
    return m.reply(`Espera, bahan tu aun no lengkap para masak *${recipe.name}*! 😭\n\nYang kurang:\n${missingMaterials.join("\n")}\n\nCari bahann primero ! 🛒🏃`);
  }

  await m.react("👨‍🍳");
  await m.reply(`Srengg... Srenggg... 🔥🍳\nLagi masak *${recipe.name}* , wangin nyebar sekampung! 🤤`);
  ...
  return m.reply(
    `TADAA! MASAKAN MATANG! 🍽️✨\n\n` +
      `Tu langsung melahap *${recipe.name}* ng enak mucho!\n` +
      `Efecto ng tu recibiste:\n` +
      `${effectTexts}\n\n` +
      `📈 Bonus EXP Masak: *+${recipe.exp}*\n\n` +
      `Kenng mucho rasan, siap tempur otra vez! 🔥`
  );
}
