...
const pluginConfig = {
  ...
  alias: ["craft", "create"],
  category: "rpg",
  description: "Fabricar items de materials",
  usage: ".craft <item>",
  example: ".craft sword",
  isOwner: false,
  isPremium: false,
...
async function handler(m, { sock }) {
  ...
  if (!itemKey) {
    let txt = `Hola Petualang! Quieres merakit alat apa ? 🛠️✨\n\n`;
  ...
  if (!recipe) {
    return m.reply(`Hayo, quieres rakit apaan tuh? Barangn no hay en daftar ! 😂\nCek list ng bener paa \`.craft\` !`);
  }
  ...
  if (missingMaterials.length > 0) {
      return m.reply(`Espera, bahann aun no cukup para ngerakit *${recipe.name}* ! 😭\n\nKekurannonn:\n${missingMaterials.join("\n")}\n\nKumpulin primero , baru balik a sesto! 🏃💨`);
  }

  await m.react("🛠️");
  await m.reply(`Tok tok tok... Krek... 🛠️🔩\nSeyg serius merakit *${recipe.name}*... Bentar otra vez jaen!`);
