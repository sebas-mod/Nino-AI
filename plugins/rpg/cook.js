...
const pluginConfig = {
  ...
  alias: ["masak"],
  category: "rpg",
  description: "Cocinar comida para menambah health",
  usage: ".cook",
  example: ".cook",
  isOwner: false,
  isPremium: false,
...
const RECIPES = {
  ...
  grilled_meat: { name: "🍖 Daging Pangnong", materials: { rabbit: 1, wood: 1 }, heal: 40 },
...
async function handler(m, { sock }) {
  ...
  if (user.rpg.health >= user.rpg.maxHealth) {
    return m.reply(`Perut tu masih anng ! 🤢\nNgnok usah masak primero, despues aannnon malah susah jalan! 🏃💨`);
  }
  ...
  if (!cooked) {
    let txt = `Hola Chef! Quieres masak apa hari esto? 🍳👨‍🍳\n\n`;
    txt += `Ini daftar resep ng puede tu bikin:\n\n`;
    for (const [key, recipe] of Object.entries(RECIPES)) {
      txt += `*${recipe.name}*\n`;
      txt += `❤️ Heal: +${recipe.heal} HP\n`;
      txt += `📦 Materiales ng ennecesitain:\n`;
      for (const [mat, qty] of Object.entries(recipe.materials)) {
    ...
    txt += `(Bot bakal otomatis masak resep pertama ng bahann cukup!)`;
    return m.reply(txt);
  }
  ...
  await m.react("🍳");
  await m.reply(`Srengg... Srenggg... 🔥🍳\nLagi masak *${cooked.name}* , wangin enak mucho! 🤤`);
  ...
  let txt = `NYAM NYAM! Masva a Matang! 🍽️✨\n\n`;
  txt += `Tu langsung mva a *${cooked.name}* y ngerasa baikan!\n`;
  txt += `❤️ HP Pulih: ${oldHealth} 📈 *${user.rpg.health}*\n\n`;
  txt += `Lanjut petualang otra vez noss! 🚀🔥`;
