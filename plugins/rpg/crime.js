...
const pluginConfig = {
  ...
  alias: ["curi", "jahat"],
  category: "rpg",
  description: "Cometer un crimen membobol ATM (risiko tinggi)",
  usage: ".crime",
  example: ".crime",
  isOwner: false,
  isPremium: false,
...
async function handler(m, { sock }) {
  ...
  await m.react("💣");
  await m.reply("Memasang alat peretas en ATM seberang jalan... 💣💻");
  ...
  if (isSuccess) {
    ...
    txt += `Mesin ATM ngeluarin duit kak air terjun! Lu langsung kabur bawa koper penuh duit.\n\n`;
    ...
    txt += `Sialan, mesinn error y polisi langsung ngepung de senola arah!\n`;
    txt += `Lu enpentung paa tongkat polisi terus enpaksa bar denda.\n\n`;
    txt += `💸 Denda Piya: *-Rp ${actualFine.toLocaleString("id-ID")}*\n`;
