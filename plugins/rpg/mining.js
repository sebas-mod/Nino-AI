...
const pluginConfig = {
  ...
  alias: ["mine", "tambang"],
  category: "rpg",
  description: "Menambang para menrecibistekan ores y gems",
  usage: ".mining",
  example: ".mining",
  isOwner: false,
  isPremium: false,
...
async function handler(m, { sock }) {
  ...
  if (user.rpg.stamina < staminaCost) {
    return m.reply(`Ay, bay tu ya remuk primeroan! 🥵\n\nNambang batu tuh berat, necesita *${staminaCost} Stamina*. Stamina tu restante *${user.rpg.stamina}* doang. Istirahat ! 🛌💤`);
  }

  user.rpg.stamina -= staminaCost;

  await m.react("⛏️");
  await m.reply("Trangg! Tranggg! ⛏️💎\nMemecah batu aras en adalaman gua...");
  ...
  txt += `Tu exitoso recibistein material esto:\n`;
  for (const r of results) {
    txt += `• ${r.name}: *+${r.qty}*\n`;
  }
  txt += `\n📈 EXP: *+${expGain}*\n`;
  txt += `⚡ Stamina: *-${staminaCost}*\n\n`;
  txt += `Simpen baik-baik  , despues puede encraft o enjual! 💎💰`;
