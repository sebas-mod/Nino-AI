...
const pluginConfig = {
  ...
  category: "rpg",
  description: "Ngojek para menrecibiste uang",
  usage: ".ngojek",
  example: ".ngojek",
  isOwner: false,
  isPremium: false,
...
async function handler(m, { sock }) {
  ...
  if (user.rpg.stamina < staminaCost) {
    return m.reply(`Mesin motorn ya panas mucho , menenng istirahat primero! 🥵🏍️💨\n\nNgojek necesita *${staminaCost} Stamina*, restante stamina tu *${user.rpg.stamina}* doang. Ngopi primero ! ☕`);
  }
  ...
  await m.reply(`Nlain motor, tarik nos! Mencari penumpang... 🏍️💨\nAda orderan *${order.type}* sejauh *${order.distance}* , Gasss! 🗺️`);
  ...
  txt += `Rincian narik ojol hari esto:\n`;
  txt += `📱 Tipe: *${order.type}*\n`;
  txt += `💵 Tarif: *+Rp ${earning.toLocaleString("id-ID")}*\n`;
  if (tips > 0) {
    txt += `🎁 Consejo Customer: *+Rp ${tips.toLocaleString("id-ID")}*\n`;
  }
  txt += `📈 EXP: *+${expGain}*\n`;
  txt += `⚡ Stamina: *-${staminaCost}*\n\n`;
  txt += `Luman para nambah-nambahin jajan ! Semannot narik otra vez despues! 🔥💪`;
