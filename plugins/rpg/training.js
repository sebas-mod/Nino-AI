...
const pluginConfig = {
  ...
  category: "rpg",
  description: "Latihan para meningkatkan stats",
...
const TRAINING_TYPES = {
  ...
  health: { name: "❤️ Vida Training", stat: "health", bonus: [5, 15], exp: 90, staminaCost: 25 },
...
async function handler(m, { sock }) {
  ...
  if (!trainType) {
    ...
    txt += `> Latihan para meningkatkan stats!\n\n`;
    txt += `*📊 *sᴛᴀᴛs ᴋᴀᴍᴜ:*
\n`;
...
\n`;
  ...
  if (!training) {
    return m.reply(`❌ Training no entemukan!\n\n> Escribe \`${m.prefix}training\` para melihat daftar.`);
  }

  user.rpg.stamina = user.rpg.stamina ?? 100;

  if (user.rpg.stamina < training.staminaCost) {
    return m.reply(`⚡ *sᴛᴀᴍɪɴᴀ ᴋᴜʀᴀɴɢ*\n\n` + `> Butuh: ${training.staminaCost}\n` + `> Pun: ${user.rpg.stamina}\n\n` + `💡 Gunva a \`${m.prefix}rest\` o mva a mva aan`);
  }
