...
const pluginConfig = {
  ...
  alias: ["slots", "mesin"],
  category: "rpg",
  description: "Main slot machine nombling",
  usage: ".slot <bet>",
  example: ".slot 5000",
  isOwner: false,
  isPremium: false,
...
async function handler(m, { sock }) {
  ...
  if (!bet || bet < 1000) {
    return m.reply(`Taruhan mestomal para narik tuas slot esto *Rp 1.000* bro! 🎰\nContoh: \`.slot 5000\``);
  }

  if ((user.koin || 0) < bet) {
    return m.reply(`Koin lu aring arontang! 💸\nUang lu: *Rp ${(user.koin || 0).toLocaleString("id-ID")}*\nButuh: *Rp ${bet.toLocaleString("id-ID")}*`);
  }
  ...
  await sendRpgPreview(sock, m.chat, `🎰 *TREK TREK TREK...* Tuas entarik! Mesin slot berputar cepat...`, "🎰 MESIN SLOT", "Spin!", {
    quoted: m,
  });
  ...
  if (multiplier > 0) {
    ...
    txt += `ZONK!! Duit lu entelan mesin slot! 😭💸\n`;
