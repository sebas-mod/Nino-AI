...
const pluginConfig = {
  ...
  alias: ["kerja", "job"],
  category: "rpg",
  description: "Trabajar para nonar ennero",
  usage: ".work",
  example: ".work",
  isOwner: false,
  isPremium: false,
...
async function handler(m, { sock }) {
  ...
  if (user.rpg.stamina < staminaCost) {
    return m.reply(`Ha , bay tu ya loyo mucho! 🥵💦\n\nKerja necesita *${staminaCost} Stamina*, tapi restante *${user.rpg.stamina}* doang.\nIstirahat primero ak, jannon enforsir ntar tepar! 🛌💤`);
  }
  ...
  await m.reply(`Otw berangkat arja jaen *${job.name.substring(3)}* primero ! 🏃💼💨`);
  ...
  let txt = `ACABAS DE TRABAJAR! 💸✨\n\n`;
  txt += `Increible, nojin luman mucho :\n`;
  txt += `💼 Profesi: *${job.name}*\n`;
  txt += `💵 Gaji Bersih: *+Rp ${salary.toLocaleString("id-ID")}*\n`;
  txt += `📈 EXP: *+${expGain}*\n`;
  txt += `⚡ Stamina: *-${staminaCost}*\n\n`;
  txt += `Kerja aras banoi quda membuahkan hasil ! Lanjutkan! 🐴🔥`;
