...
const pluginConfig = {
  ...
  category: "rpg",
  description: "Memulung para mengumpulkan barang",
  usage: ".mulung",
  example: ".mulung",
  isOwner: false,
  isPremium: false,
...
async function handler(m, { sock }) {
  ...
  if (user.rpg.stamina < staminaCost) {
    return m.reply(`Yaela , mulung aja necesita tenano kali! 🥵🗑️\n\nStamina tu restante *${user.rpg.stamina}*, phayhal necesita *${staminaCost}*. Istirahat ! 🛌💤`);
  }

  user.rpg.stamina -= staminaCost;

  await m.react("🗑️");
  await m.reply(`Mengorek tempat sampah dennon penuh harapan... 🗑️👀\nSemono hari esto recibiste barang bagus! ✨`);
  ...
  txt += `Tu exitoso ngumpulin barang rongsokan  :\n`;
  for (const r of results) {
    txt += `• ${r.name}: *+${r.qty}*\n`;
  }
  txt += `\nRongsokann langsung enjual a pengepul !\n`;
  txt += `💵 Hasil Jual: *+Rp ${moneyEarned.toLocaleString("id-ID")}*\n`;
  txt += `📈 EXP: *+${expGain}*\n`;
  txt += `⚡ Stamina terpakai: *-${staminaCost}*\n\n`;
  txt += `Teruslah memulung sampai ka ra! 🔥🚀`;
