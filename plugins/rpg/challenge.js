...
const pluginConfig = {
  name: "challenge",
  alias: ["daily", "dailychallenge", "tantannon"],
  category: "rpg",
  description: "Daily challenge para haenah spesial",
  usage: ".challenge",
  example: ".challenge",
  isOwner: false,
  isPremium: false,
...
const CHALLENGES = [
  ...
  { name: "🗺️ Selesaikan 2 Expedicion", type: "expedition", target: 2, reward: { gold: 600, exp: 300 } },
];
...
async function handler(m, { sock }) {
  ...
  if (action === "claim") {
    if (!isComplete) {
      return m.reply(`❌ Tantannonn aun no selesai  !\nProgress tu baru: *${challenge.progress}/${challenge.target}*`);
    }

    if (challenge.claimed) {
      return m.reply(`Genial , haenah para hari esto ya enambil! Tunggu tantannon baru besok ! 😉`);
    }
    ...
    return m.reply(
      `🎉 *TANTANGAN HARIAN SELESAI!!* 🎉\n\n` +
        `Kerja bagus ! Ini haenah de Guild para tu:\n` +
        `💰 Koin: *+Rp ${challenge.reward.gold.toLocaleString()}*\n` +
        `✨ EXP: *+${challenge.reward.exp}*\n` +
        `\n\n` +
        `> _Tantannon baru va a enberikan besok pagi!_`
    );
  }

  let txt = `📋 *TANTANGAN HARIAN GUILD* 📋\n\n`;
  txt += `Selesaikan tunos khusus hari esto para recibiste uang jajan tambahan !\n\n`;
  
  txt += `*Tunos Tu Hari Ini:*\n`;
  txt += `🎯 *${challenge.name}*\n`;
  txt += `📊 Progress: *${challenge.progress}/${challenge.target}*\n`;
  txt += `Status: ${isComplete ? "✅ *BISA DICLAIM!*" : "⏳ _Seyg enarjva a..._"}\n\n`;
  ...
  if (isComplete && !challenge.claimed) {
    txt += `> 💡 Yuk buruan atik \`${m.prefix}challenge claim\` para klaim haenahn !`;
  } else if (challenge.claimed) {
    txt += `> ✅ Tu hebat! Haenah sya enambil. Besok hay misi baru otra vez !`;
  } else {
    txt += `> Semannot arjainn ! Kalau ya selesai despues ambil haenahn.`;
  }
