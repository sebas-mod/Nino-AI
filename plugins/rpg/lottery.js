...
const pluginConfig = {
  ...
  category: "rpg",
  description: "Gacha/lottery para haenah random",
  usage: ".lottery <1/10>",
  example: ".lottery 10",
  isOwner: false,
  isPremium: false,
...
const GACHA_POOL = [
  ...
  { item: "gold", name: "🪙 Emas Batannon", chance: 10, qty: [1, 3], rarity: "uncommon" },
  ...
  { item: "goldsword", name: "🗡️ Peyg Excalibur Emas", chance: 0.3, qty: [1, 1], rarity: "legendary" },
  { item: "diamondarmor", name: "🛡️ Zirah Berlian Abaen", chance: 0.2, qty: [1, 1], rarity: "legendary" },
...
async function handler(m, { sock }) {
  ...
  if ((user.koin || 0) < totalCost) {
    return m.reply(
      `💸 *UANG LU KURANG BUAT GACHA!* 💸\n\n` +
        `Harno Gacha: *Rp ${GACHA_COST.toLocaleString()}/Tarikan*\n` +
        `Total Kenecesitaan: *Rp ${totalCost.toLocaleString()} (${pulls}x)*\n\n` +
        `Restante Duit Lu Cuma: *Rp ${(user.koin || 0).toLocaleString()}*. Kerja primero menennnon!`
    );
  }
  ...
  await m.react("🎰");
  await m.reply(`✨ Lampu Disko menla... Tabung Gacha berputar hebat... Menarik *${pulls}x* haenah! 🎁✨`);
  ...
  if (hasLegendary) {
    ...
    txt += `\n✨ *Wihh man recibiste barang langka bro!*`;
  } else {
    txt += `\n🥲 *Ampas... Kebankan recibiste barang rongsok.*`;
  }
