...
const pluginConfig = {
  ...
  alias: ["dadu", "roll"],
  category: "rpg",
  description: "Lempar dadu para nombling",
  usage: ".dice <1-6> <bet>",
  example: ".dice 6 5000",
  isOwner: false,
  isPremium: false,
...
async function handler(m, { sock }) {
  ...
  if (!guess || guess < 1 || guess > 6) {
    return m.reply(
      `🎲 *Bandar Dadu Jalanan* 🎲\n\n` +
        `Tebak angka ng bakal aluar! (1-6)\n\n` +
        `*Cara Main:*\n` +
        `👉 \`.dice <angka> <taruhan>\`\n\n` +
        `*Contoh:*\n` +
        `👉 \`.dice 6 5000\``
  ...
  if (!bet || bet < 1000) {
    return m.reply(`Mana hay bandar nerima taruhan segeso! Mestomal bawa *Rp 1.000* sesto! 🎲`);
  }

  if ((user.koin || 0) < bet) {
    return m.reply(`Duit lu kurang bos! Di saku solo hay *Rp ${(user.koin || 0).toLocaleString("id-ID")}*. Jannon ngutang en mari! 😤`);
  }

  user.koin -= bet;

  await sendRpgPreview(sock, m.chat, `🎲 Bandar mengocok dadu en dalam mangkok kayu... *krok krok krok*...`, "🎲 DADU JALANAN", "Rolling!", { quoted: m });
