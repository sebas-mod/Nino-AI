...
const pluginConfig = {
  ...
  category: "rpg",
  description: "Berlayg para menrecibiste hasil panen",
  usage: ".berladang",
  example: ".berladang",
  isOwner: false,
  isPremium: false,
...
async function handler(m, { sock }) {
  ...
  if (user.rpg.stamina < staminaCost) {
    return m.reply(`Ay, encok  bay! 🥵\n\nBuat nngkul necesita *${staminaCost} Stamina*, restante stamina tu *${user.rpg.stamina}*.\nMva a o istirahat primero ! 🛌💤`);
  }

  user.rpg.stamina -= staminaCost;

  await m.react("🌾");
  await m.reply(`Menyiapkan cangkul y menyiram tanah... 🌱💧\nSemono hari esto panen berlimpah!`);
  ...
  txt += `Increible, abun tu hari esto nnosilin bank mucho:\n`;
  ...
  txt += `Tu puede jual hasil panen esto a pasar pakai \`${m.prefix}sellall\` ! 🧺✨`;
