...
const pluginConfig = {
  ...
  alias: ["nahidatts"],
  category: "tts",
  description: "Texto a voz con la voz de Nahida",
  usage: ".ttsnahida <texto>",
  example: ".ttsnahida Hola viajero!",
  isOwner: false,
  isPremium: false,
  isGroup: false,
  isPrivate: false,
...
async function handler(m, { sock }) {
  ...
  try {
    ...
    if (!voice) {
      m.react("❌");
      return m.reply(
        `❌ Error de voz de Nahida. Prueba \`${m.prefix}ttsgoku\` o \`${m.prefix}ttsnami\``,
      );
    }
    ...
    const audioRes = await axios.get(voice.nahida, {
