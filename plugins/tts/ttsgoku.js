...
const pluginConfig = {
  ...
  alias: ["gokutts"],
  category: "tts",
  description: "Texto a voz con la voz de Goku",
  usage: ".ttsgoku <texto>",
...
async function handler(m, { sock }) {
  ...
  if (!text)
    return m.reply(`🐉 *ɢᴏᴋᴜ ᴛᴛs*\n\n> Usa: \`${m.prefix}ttsgoku <texto>\``);
  ...
  try {
    ...
    if (!voice) {
      m.react("❌");
      return m.reply(`❌ Error de voz de Goku. Prueba otro TTS.`);
    }
    ...
    const audioRes = await axios.get(voice.goku, {
