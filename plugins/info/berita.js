...
const pluginConfig = {
    ...
    alias: [],
    category: 'berita',
    description: 'Obtener noticias recientes de varias fuentes',
    usage: '.berita <fuente>',
...
async function handler(m) {
    ...
    if (cmd === 'berita') {
        ...
        if (!arg) {
            ...
            txt += `\n_O usa: \`${m.prefix}berita <fuente>\`_`
            return m.reply(txt)
        }
    ...
    if (!newsSource) {
        return m.reply(`❌ Fuente de noticias no valida.`)
    }
    ...
    try {
        ...
        txt += `━━━━━━━━━━━━━━━\n`
        txt += `_Total: ${articles.length} articulos disponibles_`
