...
const pluginConfig = {
  ...
  category: "info",
  description: "Ver calendario de partidos de futbol",
...
const NEOXR_APIKEY = config.APIkey?.neoxr || "Milik-Bot-NinoMD";
...
async function handler(m, { sock }) {
  ...
  try {
    ...
    if (matches.length === 0) {
      m.react("❌");
      return m.reply(`❌ No se encontro calendario para: \`${filter}\``);
    }
